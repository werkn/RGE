...
/* Example usage
    arraystack_t allegro_quiz;
    iterator_t it;
    arraystack_init(&allegro_quiz, sizeof(quiz_t));

    arraystack_push(&allegro_quiz, &tmp_quiz);

    arraystack_iterator(&allegro_quiz, &it, 1, NUM_OF_UNITS);

    while (it.next(&it)) {
        quiz_t tmp = *(quiz_t*)it.elem(&it);
        printf("%s\n", tmp.questions[1].question);
    }
    //it.dispose(&it);
    arraystack_dispose(&allegro_quiz);
    */
...
void* arraystack_get_pointer(arraystack_t* stack, int index) {

    assert((void*)stack != NULL);
    assert(stack->length > 0);
    assert(index < stack->length);
    
    return (char*)stack->array + (stack->elem_size * index);
}
